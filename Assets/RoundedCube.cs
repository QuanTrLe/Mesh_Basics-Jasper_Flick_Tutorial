...
public class RoundedCube : MonoBehaviour
{
    ...
    private void CreateTriangles()
    {
        ...
        int[] trianglesZ = new int[(xSize * ySize) * 12]; // splitting the mesh for no seam duplication while techturing
        int[] trianglesX = new int[(ySize * zSize) * 12];
        int[] trianglesY = new int[(zSize * xSize) * 12];
        ...
        {
            for (int q = 0; q < xSize; q++, v++) // wrapping around the ring except for last quad
            {
                tZ = SetQuad(trianglesZ, tZ, v, v + 1, v + ring, v + ring + 1);
            }
            for (int q = 0; q < zSize; q++, v++) // alternate between Z and X
            {
    ...
    private int CreateTopFace (int[] triangles, int t, int ring) {
        ...
        {
            t = SetQuad(triangles, t, vMin, vMid, vMin - 1, vMid + xSize - 1); // separate since dealing with the outer ring (separate vertices set)
            ...
            t = SetQuad(triangles, t, vMid, vMax, vMid + xSize - 1, vMax + 1); // the final quad that connects to ring vertices
        }
    ...
    private int CreateBottomFace(int[] triangles, int t, int ring)
    {
        ...
        for (int z = 1; z < zSize - 1; z++, vMin--, vMid++, vMax++)
        {
            t = SetQuad(triangles, t, vMin, vMid + xSize - 1, vMin + 1, vMid); // first quad in row
            for (int x = 1; x < xSize - 1; x++, vMid++)
            { // middle quads in row
                t = SetQuad(
                    triangles, t,
            ...
            t = SetQuad(triangles, t, vMid + xSize - 1, vMax + 1, vMid, vMax); // last quad in row
        }
